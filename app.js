    ...
    drinks
      ...
      .then(data => {
        if (data) {
          ...
          //1. based on type
          if (searchType !== "All") {
            data.drinks = data.drinks.filter(
              drink =>
                drink.strAlcoholic.toLowerCase() == searchType.toLowerCase()
            );
          }
          ...
          if (searchCatgy !== "All") {
            data.drinks = data.drinks.filter(
              drink =>
                drink.strCategory.toLowerCase() == searchCatgy.toLowerCase()
            );
          }
          console.log(data.drinks);
          if (data.drinks.length == 0) ui.showErr(search);
          else {
            ui.paint(data.drinks);
          }
        } else {
          ui.showErr(search);
        }
